...
export function DevicesPage() {
  ...
  const selected = filtered.find((device) => device.id === selectedId) ?? filtered[0]
  ...
  const compatibilityCount = selected ? catalog.compatibilities.filter((edge) => edge.deviceId === selected.id && ['verified', 'fits'].includes(edge.status)).length : 0
  ...
  function updateFilters(values: { market?: Market; ecosystem?: Ecosystem | 'all'; device?: string }) {
    ...
    if (values.ecosystem) next.delete('device')
