...
export function ComparePage() {
  ...
  const requested = [...new Set(params.get('devices')?.split(',').filter((id) => catalog.devices.some((device) => device.id === id)) ?? [])].slice(0, 4)
  const selected = params.has('devices') ? requested : catalog.devices.slice(0, 3).map((device) => device.id)
