...
export function ComparePage() {
  ...
  return <>
    ...
    <section className="compare-picker"><h2>{locale === 'tr' ? 'Cihaz seç' : 'Select devices'}</h2><div>{catalog.devices.map((device) => <label key={device.id}><input type="checkbox" checked={selected.includes(device.id)} disabled={!selected.includes(device.id) && selected.length >= 4} onChange={(event) => toggle(device.id, event.target.checked)} /><span>{device.name}</span></label>)}</div></section>
    <section className="compare-section"><p className="eyebrow">01 / DEVICE SKU</p><h2>{locale === 'tr' ? 'Dört cihazlık matris' : 'Four-device matrix'}</h2><div className="table-scroll"><table className="compare-table"><thead><tr><th>{locale === 'tr' ? 'Ölçüt' : 'Measure'}</th>{devices.map((device) => <th key={device.id}>{device.name}</th>)}</tr></thead><tbody><tr><th>{locale === 'tr' ? 'Ekosistem' : 'Ecosystem'}</th>{devices.map((device) => <td key={device.id}>{device.ecosystem.toUpperCase()}</td>)}</tr><tr><th>{locale === 'tr' ? 'Bellek' : 'Memory'}</th>{devices.map((device) => <td key={device.id}>{device.memory.totalGiB} GB</td>)}</tr><tr><th>{locale === 'tr' ? 'Güvenli sınır' : 'Safe boundary'}</th>{devices.map((device) => <td key={device.id}>{device.memory.usableGiB ?? device.memory.totalGiB * .8} GB</td>)}</tr><tr><th>{locale === 'tr' ? 'Bant genişliği' : 'Bandwidth'}</th>{devices.map((device) => <td key={device.id}>{device.bandwidthGBs ? `${device.bandwidthGBs} GB/s` : '—'}</td>)}</tr><tr><th>Runtime</th>{devices.map((device) => <td key={device.id}>{device.runtimes.join(', ')}</td>)}</tr><tr><th>{locale === 'tr' ? 'Form' : 'Form'}</th>{devices.map((device) => <td key={device.id}>{device.formFactor}</td>)}</tr></tbody></table></div></section>
    <section className="compare-section"><p className="eyebrow">02 / LAB PACKAGE</p><h2>{locale === 'tr' ? 'Üç paketlik karşılaştırma' : 'Three-package comparison'}</h2><div className="package-grid">{presets.map((preset) => { const scenario = { ...defaultScenario, budget: preset.budget, ownedDeviceIds: preset.ownedDeviceIds }; const result = recommendLab(scenario, catalog); return <article key={preset.id}><p className="eyebrow">{preset.id}</p><h3>{preset[locale]}</h3><strong>{new Intl.NumberFormat('tr-TR', { style: 'currency', currency: 'TRY', maximumFractionDigits: 0 }).format(result.totalCost)}</strong><dl><div><dt>{locale === 'tr' ? 'Durum' : 'Status'}</dt><dd>{result.status}</dd></div><div><dt>{locale === 'tr' ? 'En zayıf fit' : 'Weakest fit'}</dt><dd>{result.weakestFitScore}/100</dd></div><div><dt>{locale === 'tr' ? 'Kapsam' : 'Coverage'}</dt><dd>{result.workloadCoverage}/100</dd></div></dl><p>{result.slots.map((slot) => catalog.devices.find((device) => device.id === slot.deviceId)?.name).join(' · ')}</p></article>})}</div><Link className="button button--primary" to={`/${locale}/build`}>{locale === 'tr' ? 'Kendi paketini oluştur' : 'Build your package'}</Link></section>
  </>
}
